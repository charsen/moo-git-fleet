...
> 当前状态：普通用户不再同时面对继续、合并、拆分与主线确认

### 113.1 本轮边界

- 普通模式把多设备分叉表述为“不同电脑上的工作进度”，只要求用户选择现在要继续的一份。
- 选择后自动读取并准备对应内容；其他进度继续保留，不要求用户理解或确认 HEAD、恢复基线和逻辑主线。
- 合并交接、拆成两个会话、确认唯一主线及分支/提交标识统一只在“管理与历史”模式出现。
- 不自动覆盖、合并或删除任何分叉对象；分叉选择、合并、拆分的服务端合同与并发守门保持不变。

### 113.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 分叉决策面审计 | 已完成 | 默认详情一次展示继续、合并、拆分三种处理方式，以及 HEAD、分支、提交和“确认主线”，普通恢复需要理解完整版本图 | 临时选择一份继续已能触发自动准备；合并、拆分和主线确认均属于低频管理动作 |
| 2026-07-30 | F0 普通工作线选择 | 已完成 | 普通模式改为“工作线 01/02”，隐藏三类管理动作与技术标识；选择结果使用设备和任务语言，并自动进入恢复准备 | 管理模式完整保留原三种处理方式、HEAD 元数据与确认主线按钮；未选时仍不会自动恢复 |
| 2026-07-30 | R0 最小门禁 | 已完成 | 跳过测试矩阵与浏览器验收，仅检查类型、生产构建和差异格式 | `npm run typecheck`、`npm run build`、`git diff --check` 通过 |

## 114. 恢复页默认层改为任务语言

> 当前状态：普通恢复路径不再直接展示 WIP、cmux 与原生恢复术语

### 114.1 本轮边界

- 恢复主动作只显示“下一步、立即继续 / 复制终端指令”，标准权限说明改为“安全启动，不改动现有代码”。
- 原“恢复设置与技术详情”改为“更多选项”，折叠说明使用“项目目录、启动方式和技术信息”；具体分支、WIP、原生恢复和 cmux 术语仍在展开后保留。
- cmux 不可用时复制成功明确提示“请在项目终端中粘贴运行”，避免用户拿到指令后不知道下一步。
- 接着工作模式的“退出排序”改为“返回普通列表”，直接描述用户将回到哪里。

### 114.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 默认恢复语言审计 | 已完成 | 折叠入口仍罗列 WIP、原生恢复、权限与 cmux；复制按钮只反馈“恢复指令已复制”，没有告诉用户如何执行 | 技术能力已全部位于折叠内容，默认层只需表达任务动作与复制后的明确去向 |
| 2026-07-30 | F0 任务语言与复制引导 | 已完成 | 默认主动作、权限说明、折叠入口和返回按钮改为用户任务语言；剪贴板函数返回结果，复制终端指令成功后给出粘贴运行提示 | 高级功能、命令内容和危险权限提示未删除；其他复制动作仍使用原标签反馈 |
| 2026-07-30 | R0 最小门禁 | 已完成 | 跳过测试矩阵与浏览器验收，仅检查类型、生产构建和差异格式 | `npm run typecheck`、`npm run build`、`git diff --check` 通过 |

## 115. 可撤销归档不再确认

> 当前状态：归档单击执行，只有废纸篓动作保留确认

### 115.1 本轮边界

- 归档是可撤销、不会删除内容的管理动作，改为单击立即执行，并继续通过结果反馈提供“撤销”。
- 移入废纸篓会从列表隐藏并进入保留期，仍保留确认弹窗。
- 废纸篓确认只说明 30 天保留、跨电脑同步、可恢复和不删除原始会话/源码，不再混合归档分支和生命周期事件术语。
- 生命周期版本、远端未合并锁定、审计事件、过滤列表刷新与撤销合同保持不变。

### 115.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 管理确认审计 | 已完成 | 归档和废纸篓共用一套确认弹窗；归档本身可恢复且成功反馈已有撤销按钮，额外确认不降低实际风险 | 仅废纸篓需要在隐藏与保留期前再次确认，归档可复用现有直接生命周期 mutation |
| 2026-07-30 | F0 按可逆性执行 | 已完成 | `requestLifecycle` 仅对 trash 打开确认，archive/restore/pin 等直接执行；确认弹窗收敛为单一废纸篓内容 | 归档仍受 lifecycleLocked 与版本校验保护，失败重试和成功撤销保持原行为 |
| 2026-07-30 | R0 最小门禁 | 已完成 | 跳过测试矩阵与浏览器验收，仅检查类型、生产构建和差异格式 | `npm run typecheck`、`npm run build`、`git diff --check` 通过 |

## 116. 历史仓库流程去技术化

> 当前状态：普通用户只需选择新仓库位置，按需开启跨电脑同步，然后一次开始

### 116.1 本轮边界

- 管理入口固定使用“历史仓库”，当前仓库与历史仓库不再暴露 Epoch、Vault、HEAD、checkpoint、Pull 或 Push 等底层术语。
- “开始新仓库”只保留新位置和可选私有 Git 地址；删除私有远端确认短语输入，由前端在提交时使用既有安全确认常量。
- 操作结果压缩为三条：当前仓库保留为历史、新会话进入新仓库、已有历史不会被删除或改写。
- 历史查看页只表达“查看、导出、返回当前仓库”，不再常驻展示版本哈希和内部只读实现。
- 服务端轮换接口、空私有远端验证、期望当前仓库版本和只读归档确认保持不变。

### 116.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 历史仓库流程审计 | 已完成 | 原弹窗同时要求理解纪元、Vault、HEAD、Remote、确认短语和四条底层保证，查看历史时仍显示归档版本与同步术语 | 用户真正需要的决定只有新保存位置和是否跨电脑同步，底层保护可继续由现有接口承担 |
| 2026-07-30 | F0 单路径开始新仓库 | 已完成 | 统一改为当前仓库、历史仓库、开始新仓库；移除确认短语输入和版本哈希常驻展示；按钮改为“同步当前内容并开始” | 远端开启时仍必须填写全新的空私有 Git 地址，提交参数与后端安全守门未放宽 |
| 2026-07-30 | R0 加速交付 | 已跳过 | 按用户要求暂不执行类型、构建、测试矩阵和浏览器验收 | 仅检查差异格式与残留引用 |

## 117. 管理页去除无动作统计

> 当前状态：进入管理后直接筛选和处理会话，不再先阅读一排技术统计

### 117.1 本轮边界

- 删除管理页顶部“逻辑会话、本机待同步、代码不可达、已分叉”四格统计；总数已有生命周期标签，异常会在对应会话上直接出现。
- 入口从“管理与历史”改为“管理会话”，历史仓库保留独立按钮，避免一个入口混合两项业务。
- 没有远端新内容时不再显示不可点击的“拉取更新”；确实存在新内容时才出现“获取新内容”。
- 同步未处理时统一提示先完成同步，不再让不可执行状态继续指向一个禁用按钮。

### 117.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 管理页动作密度审计 | 已完成 | 四格统计占用 74px 高度，使用 Vault commit、checkpoint、逻辑会话等术语，却不能直接触发处理；拉取按钮无更新时仍长期显示禁用态 | 生命周期标签已有完整计数，分叉和代码异常已有逐条提示，顶部统计属于重复信息 |
| 2026-07-30 | F0 直接进入管理任务 | 已完成 | 删除统计计算、模板、样式和响应式规则；拆清“管理会话 / 历史仓库”；拉取动作按真实新内容出现 | 自动拉取、父级可用性通知和同步 API 保持原逻辑，只减少无动作状态的界面占用 |
| 2026-07-30 | R0 加速交付 | 已跳过 | 按用户要求暂不执行类型、构建、测试矩阵和浏览器验收 | 仅检查差异格式与残留引用 |

## 118. 删除后新内容一键保留

> 当前状态：用户只决定恢复原会话还是保留新内容，不再重新填写一份交接表单

### 118.1 本轮边界

- “删除后又收到新内容”保留两个真实业务结果：恢复原会话，或让原会话继续留在废纸篓并把新内容保留为新会话。
- 保留新内容时默认采用最近保存的一份；只有检测到多份内容才显示选择，不再要求理解 checkpoint、分支、哈希和 lineage。
- 删除新会话目标与下一步两个必填字段，系统根据所选内容标题自动生成最小可继续摘要。
- 成功反馈只报告处理结果，不再附加内部会话 ID；服务端版本校验、冲突集合校验和审计记录保持不变。

### 118.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 删除冲突决策审计 | 已完成 | 用户已经选择“另存新会话”后，弹窗仍要求选择 checkpoint、填写目标和至少一条下一步，并阅读交接对象与 lineage 说明 | 最近新增内容已有稳定排序和标题，足以生成安全的最小摘要；真正必须保留的只有两种处置结果 |
| 2026-07-30 | F0 自动生成新会话摘要 | 已完成 | 单份新增内容直接确认，多份才选择；移除目标/下一步状态、校验和表单；文案统一为“恢复原会话 / 保留新内容” | API 仍提交完整人工复核格式摘要，原会话不会自动恢复，其他新增内容不会删除，项目源码不受影响 |
| 2026-07-30 | R0 加速交付 | 已跳过 | 按用户要求暂不执行类型、构建、测试矩阵和浏览器验收 | 仅检查差异格式与残留引用 |

## 119. 管理详情默认只看任务

> 当前状态：管理模式也先显示当前目标和接下来，技术记录按需展开

### 119.1 本轮边界

- 管理详情不再默认铺开来源、分支、工作区、代码状态、完整 Markdown 和保存时间线。
- 当前目标与接下来成为所有模式一致的默认摘要，完整交接记录继续保留在原有展开入口中。
- 来源、分支、工作区、代码状态和保存时间线合并到默认收起的“技术记录”，需要排障或审计时再展开。
- 加载、内容已清理和未选择进度等提示改成任务语言，删除 Vault HEAD、checkpoint 等默认层术语。

### 119.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 管理详情信息层级审计 | 已完成 | 打开管理详情后首先出现四格英文技术状态和完整交接 Markdown，底部继续展示 checkpoint 时间线；真正的目标与下一步反而只在普通模式出现 | 管理动作不依赖这些内容常驻展示，现有完整摘要与技术字段均可安全移入按需展开层 |
| 2026-07-30 | F0 任务优先详情 | 已完成 | 所有模式统一使用目标/下一步摘要；新增“技术记录”折叠区承载原状态与时间线；普通提示去除底层存储术语 | 原始 Markdown、设备、分支、代码可达性、版本 ID 和时间线均未删除，管理与审计能力保持完整 |
| 2026-07-30 | R0 加速交付 | 已跳过 | 按用户要求暂不执行类型、构建、测试矩阵和浏览器验收 | 仅检查差异格式与残留引用 |

## 120. 默认提示统一使用任务语言

> 当前状态：废纸篓、空状态、同步错误和首次设置不再要求理解底层存储模型

### 120.1 本轮边界

- 列表筛选统一使用“AI 类型、会话状态”，管理说明改成“整理交接记录”，不再显示 Provider、生命周期和 Vault。
- 废纸篓列表、详情、恢复禁用提示与清理预览统一使用“交接内容、列表记录、历史仓库或备份”。
- 首次设置的目录与 Remote 错误改为“保存位置、连接名称”，同步失败和并发变化提示直接告诉用户下一步。
- 管理标题和未关联分支使用普通表达；高级分叉、权限和命令模板仍保留必要技术术语。

### 120.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 默认文案残留审计 | 已完成 | 主结构压缩后，空状态、废纸篓与错误分支仍混用 Vault、Provider、生命周期、Remote、工作树和索引等词 | 这些词不参与用户决策，可直接映射到已有业务概念而不改变行为 |
| 2026-07-30 | F0 任务语言收尾 | 已完成 | 更新首次设置错误、同步错误、筛选说明、废纸篓状态与清理提示；管理标题与未关联分支去内部枚举 | 高级技术入口、服务端原始错误、审计字段和所有 API 参数保持不变 |
| 2026-07-30 | R0 加速交付 | 已跳过 | 按用户要求暂不执行类型、构建、测试矩阵和浏览器验收 | 仅检查差异格式与残留引用 |

## 121. 多电脑进度改为结果选择

> 当前状态：先选择要继续的进度，只有需要长期整理时才合成或分开保存

### 121.1 本轮边界

- 分叉默认层统一为“不同电脑上的工作进度”，不再展示 HEAD、分支哈希、恢复基线和三张技术处理卡。
- 日常只需选择一份进度继续；管理模式可将所选进度设为以后默认使用。
- “合成一条会话 / 分开保存”收进按需展开的“整理其他进度”，分别对应原合并与拆分服务端能力。
- 两个处理弹窗删除目标和下一步必填表单，改为展示将保留什么、之后从哪里继续，由系统生成完整摘要。
- 合成仍不自动合并项目代码；分开仍仅支持两份当前进度；期望版本、当前进度集合与同步状态校验保持不变。

### 121.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 分叉业务合同审计 | 已完成 | 服务端实际只需要所选进度、当前进度集合和完整摘要，前端却要求用户理解 HEAD、lineage、合并点，并在合并与拆分时重复填写目标和下一步 | 所选进度和现有标题足以生成最小摘要，底层并发与同步守门可独立保留 |
| 2026-07-30 | F0 结果导向分叉处理 | 已完成 | 所有模式统一进度卡；管理整理按需展开；移除六个表单/选择状态与两组输入；确认页改为所选结果预览 | 选择、合并、拆分三个 API 合同未改；其他进度不删除，项目代码不自动合并，成功反馈去掉内部 ID |
| 2026-07-30 | R0 加速交付 | 已跳过 | 按用户要求暂不执行类型、构建、测试矩阵和浏览器验收 | 仅检查差异格式与残留引用 |

## 122. 废纸篓清理只显示执行结果

> 当前状态：确认页只回答会清理多少、释放多少、现在能不能执行

### 122.1 本轮边界

- 删除废纸篓清理确认页的六格统计、常驻同步状态和重复历史警告。
- 可执行时只展示到期条数、预计释放空间和仍在保留期的条数；确认按钮直接带出清理数量。
- 不可执行时只显示“没有到期内容”或“暂时不能清理”，已有服务端阻塞原因继续逐条保留。
- 清理成功反馈改为已清理条数与释放空间；指纹校验、同步守门、保留期和审计记录保持不变。

### 122.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 清理确认信息审计 | 已完成 | 原页面同时展示总数、到期、保留、分叉、删除冲突、对象数、同步信息、阻塞与历史警告，用户需自行判断是否能执行 | `canEmpty`、`eligibleSessions`、`removableBytes` 和 `blockers` 已能直接表达决定与结果 |
| 2026-07-30 | F0 单结果清理确认 | 已完成 | 可执行与阻塞状态各收敛为一个摘要；按钮动态显示清理数量；成功反馈去技术化；删除旧统计与同步展示样式 | 服务端预览与清理请求未改，不能清理时按钮继续禁用，破坏性操作仍保留确认 |
| 2026-07-30 | R0 加速交付 | 已跳过 | 按用户要求暂不执行类型、构建、测试矩阵和浏览器验收 | 仅检查差异格式与残留引用 |

## 123. 更换保存会话只保留辨认信息

> 当前状态：系统默认自动选择；选错时只根据标题、项目和时间更换

### 123.1 本轮边界

- 保持打开保存抽屉后自动选择最近且可保存会话，不增加确认步骤。
- 手动更换列表删除 session id、消息数量、文件大小、JSONL、尾行截断和 Fleet 仓库等诊断表达。
- 候选项只显示 AI 类型、标题、项目、最近时间和必要不可用原因；推荐状态改为“最近使用”。
- 已选会话改为“已自动选择 / 更换”；正常代码处理结果压成一行，只有代码风险继续展示完整说明。

### 123.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 保存选择流程审计 | 已完成 | 自动选择已存在，但手动列表仍按诊断工具展示 session id、消息数、字节数、JSONL、仓库关联和截断状态；安全代码结果仍占完整卡片 | 更换会话只需标题、项目、AI 类型和最近时间，其余信息不参与选择 |
| 2026-07-30 | F0 任务化候选与代码结果 | 已完成 | 搜索和候选文案去技术化；移除候选数量/大小展示与未使用格式化函数；安全代码状态改成紧凑单行，风险状态保持原卡片 | 自动推荐、可读性与项目关联守门、源码处理选项和保存 payload 均未改变 |
| 2026-07-30 | R0 加速交付 | 已跳过 | 按用户要求暂不执行类型、构建、测试矩阵和浏览器验收 | 仅检查差异格式与残留引用 |

## 124. 仓库状态筛选合并为一个入口

> 当前状态：顶部保留常用状态快捷入口，列表工具栏只保留一个完整状态筛选

### 124.1 本轮边界

- 顶部摘要继续提供全部、今日待处理、工作区改动、待推送和待拉取五个常用入口。
- 删除列表工具栏中重复常驻的七个状态按钮，改为一个状态下拉框。
- 有动静和久未 Fetch 等低频状态仍可选择，每个状态继续显示当前仓库数量。
- 搜索、分组、排序、筛选记忆、结果重置与批量操作范围均不改变。

### 124.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 重复筛选审计 | 已完成 | 顶部已有五个高频状态快捷入口，列表标题右侧又常驻七个同义按钮，挤占宽度并要求用户重复理解两套入口 | 两套入口共用同一筛选状态，低频条件可安全收进单一完整入口 |
| 2026-07-30 | F0 单入口状态筛选 | 已完成 | 七按钮替换为状态下拉框；删除独立键盘游标状态和横向滚动样式；移动端工具栏改为单列排列 | 七种状态、数量、筛选记忆、顶部快捷入口和重置逻辑全部保留 |
| 2026-07-30 | R0 加速交付 | 已跳过 | 按用户要求暂不执行类型、构建、测试矩阵和浏览器验收 | 仅检查差异格式与残留引用 |

## 125. 批量操作范围由当前页面自动决定

> 当前状态：有筛选就处理当前结果，没有筛选就处理全部仓库

### 125.1 本轮边界

- 删除批量操作前“当前结果 / 全部仓库”的额外范围选择。
- 页面存在搜索、分组或状态条件时，批量操作自动作用于当前结果；没有条件时自动作用于全部仓库。
- 工具栏保留一行只读提示，明确显示实际范围和目标仓库数。
- Pull、Push 确认页继续展示范围、可执行数和跳过数；服务端安全复核与操作队列不改变。
- 旧版范围偏好字段仅为配置兼容保留，不再参与目标集合计算。

### 125.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 批量范围决策审计 | 已完成 | 用户已通过筛选表达处理范围，工具栏仍要求再次选择当前结果或全部仓库；未筛选时还显示一个不可产生新结果的单选按钮 | 当前页面条件足以安全推断目标集合，确认页和服务端仍可承担最终复核 |
| 2026-07-30 | F0 自动范围与只读提示 | 已完成 | 删除范围状态、切换按钮和持久化监听；目标集合直接由是否存在筛选决定；工具栏显示实际范围和数量 | 默认行为与原“当前结果”一致；清空条件即可处理全部；Pull、Push 确认和所有 API 参数保持不变 |
| 2026-07-30 | R0 加速交付 | 已跳过 | 按用户要求暂不执行类型、构建、测试矩阵和浏览器验收 | 仅检查差异格式与残留引用 |

## 126. 同步确认只说明结果和安全边界

> 当前状态：确认时只看执行数量、自动跳过数量和不会发生什么

### 126.1 本轮边界

- 批量 Pull、Push 不再展示操作队列、服务端复核、fast-forward、refspec 和 force push 等实现表达。
- 确认页直接显示将执行多少个仓库、多少个会自动跳过，以及不会自动合并或强制覆盖。
- 单仓库 Pull、Push 改为说明更新去向和遇到远端变化时的处理结果。
- 不可执行提示改为本地改动、未关联远端、不能直接更新或没有新提交等用户可处理原因。
- 通用确认弹窗删除英文副标题与重复安全提示，操作目标改为“本次范围”，按钮与安全守门不改变。

### 126.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 同步确认信息审计 | 已完成 | 批量和单仓库确认同时解释队列、fast-forward、服务端、Fetch 复核、refspec 与 force，弹窗头尾还重复展示英文检查点和服务端安全校验 | 用户决定是否继续只需要知道实际范围、自动跳过和代码不会被自动合并或覆盖 |
| 2026-07-30 | F0 结果导向同步确认 | 已完成 | 批量确认改为执行数/跳过数；单仓库确认改为更新去向；成功提示去除队列术语；压缩确认弹窗头尾 | Pull、Push 请求、二次状态检查、禁止自动合并和禁止强制推送的行为均未改变 |
| 2026-07-30 | R0 加速交付 | 已跳过 | 按用户要求暂不执行类型、构建、测试矩阵和浏览器验收 | 仅检查差异格式与本轮移除文案引用 |

## 127. AI 会话回归本地管理与双机备份

> 当前状态：产品主模型收敛为“本机会话 GUI + 私有 Git 双机备份恢复”，用户只需点击一次同步

### 127.1 本轮边界

- 主页面只管理本机 Claude/Codex 会话：自动发现、搜索、查看真实对话和删除。
- 项目源码继续使用项目自己的 Git；会话同步不再处理源码、WIP、交接摘要或项目仓库绑定。
- “备份全部”和“拉取同步”合并为一个“同步会话”：先接收另一台电脑的更新，再备份本机全部变化，最后统一上传。
- 完整 JSONL 记录的安全捕获与当前机器 CLI 探测解耦；CLI 暂时不可用时仍可备份，恢复时再验证兼容性。
- 同步后的多版本先比较完整记录：完全相同自动跳过，前缀关系自动保留更完整版本，真正分叉才要求选择。
- 删除默认仅移到本机系统废纸篓并保留同步备份；移出同步备份必须显式勾选，另一台电脑已有本地文件不会被静默删除。
- 另一台电脑的新会话提供“全部恢复”；需要目录选择、覆盖或版本处理的异常项继续逐条确认。

### 127.2 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | D0 产品模型复盘 | 已完成 | 对照编程小白双机开发场景，确认旧方案把同步工具扩张成接力平台；重新定义为本机会话管理器和私有 Git 备份通道 | 主路径不再要求理解 Vault、checkpoint、epoch、lineage、WIP 或 capsule |
| 2026-07-30 | F1 安全删除语义 | 已完成 | `LocalSessionDeleteRequest` 新增默认关闭的跨设备移除选项；普通删除只移动本机 JSONL 到系统废纸篓，备份继续保留 | 删除弹窗明确区分本机与两台电脑范围，旧同步删除重试仍兼容 |
| 2026-07-30 | F2 完整记录独立捕获 | 已完成 | 原生记录新增 `captured` 状态；CLI 探测失败不再阻止安全可读 JSONL 进入备份，目标机恢复时再验证 provider 能力与可用版本 | checkpoint 写入、读取、去重、拆分和恢复链均支持 `captured` |
| 2026-07-30 | F3 前缀自动对齐 | 已完成 | 新增 `session-reconcile.ts`；同步后读取各 head 的完整记录，唯一包含其他版本的记录自动成为保留版本，相同内容按稳定顺序收敛 | 读取失败、缺少完整记录或真实分叉不会自动处置，继续留给 GUI 三选一 |
| 2026-07-30 | F4 单一同步动作 | 已完成 | 备份抽屉由 613 行逐条预览/选择流程重写为 323 行自动进度抽屉；顶部与页面均统一为“同步会话”，内部顺序为 Pull → 全量变化备份 → 单次 Push | 本机会话内容预览仍在详情抽屉；同步失败时本机备份保留并可重试 |
| 2026-07-30 | F5 批量恢复 | 已完成 | 另一台电脑的新会话新增“全部恢复”；只自动执行项目路径已匹配且无需覆盖的安全项，异常项保留逐条入口 | 常见双机路径无需逐条点击，未知目录、替换和分叉不会被静默处理 |
| 2026-07-30 | R0 轻量门禁 | 已完成 | 按加速要求跳过完整测试和浏览器矩阵，执行类型检查与差异格式检查 | `npm run typecheck`、`git diff --check` 通过 |

## 128. 会话同步核心重写：删掉接力平台，只留备份与对齐

> 当前状态：v0.3 遗留机器整体下线，会话子系统非测试代码 11,988 行 → 1,897 行

### 128.1 本轮判断

第 127 节把**产品模型**收敛到了「本机会话 + 一次同步」，但**实现**仍然是 v0.3 的整套机器：checkpoint 事件流、epoch 纪元、lineage 接力链、handoff 摘要、WIP ref、cmux、native capsule、probe 探测都还在，只是被 UI 挡住了。此前连续约 30 个 `feat: simplify …` 提交都在 UI 层做减法，复杂度只是从可见变成隐藏，总代码量反而上升。本轮把底座换掉。

关键认识：**本机 JSONL 是真相，备份仓是派生副本**。由此得到三条简化：

1. 同步第一步可以直接把备份仓 `reset --hard` 到远端——本机内容随后会重新写上去，推送永远快进，Git 层不需要做内容合并，也就不存在冲突标记。
2. 会话文件只追加、不改写已有行，所以「谁更全」逐行比前缀即可，不需要 diff 算法，也不需要事件流 / checkpoint / lineage 来表达版本关系。
3. 跨电脑的项目定位复用仓库注册表的规范化远端（`projectId`），同一远端在两台电脑上必然同值，不需要任何映射表。

### 128.2 新的模块与接口

| 模块 | 行数 | 职责 |
| --- | --- | --- |
| `sessions/compare.ts` | 110 | 逐行前缀比对 → same / local-only / backup-only / local-ahead / backup-ahead / diverged / backup-deleted |
| `sessions/backup-store.ts` | 190 | `sessions/<provider>/<id>.jsonl` + `.json`（含删除墓碑）的读写 |
| `sessions/backup-repo.ts` | 295 | 备份仓绑定、初始化、fetch / commit / push；保留「不在 Fleet 源码仓内 / 不嵌套其他仓 / 远端无凭据」三条安全检查 |
| `sessions/sync-run.ts` | 440 | 一键同步编排、三选处理、删除与墓碑 |
| `sessions/local-sessions.ts` | 120 | 本机列表与内容预览 |

接口从 37 条收敛为 7 条：`GET/POST /api/session-backup(/initialize)`、`GET /api/local-sessions`、`GET /api/local-sessions/:provider/:id`、`POST /api/local-sessions/:provider/:id/trash`、`POST /api/session-sync`、`POST /api/session-sync/resolve`。

### 128.3 执行与验证

| 日期 | 步骤 | 状态 | 业务与代码回填 | 验证结果 |
| --- | --- | --- | --- | --- |
| 2026-07-30 | S1 比对与存储 | 已完成 | 新增 `compare.ts` / `backup-store.ts`；备份仓结构由 events + objects 目录改为一条会话一对文件 | 21 个单测覆盖前缀、墓碑、路径穿越防护 |
| 2026-07-30 | S2 备份仓与同步 | 已完成 | 新增 `backup-repo.ts` / `sync-run.ts`；同步顺序固定为 fetch → 对齐远端 → 比对 → 自动处理 → 单次 commit/push | 14 个双机往返用例：备份、恢复、追加、真分叉三选、删除墓碑、跟随删除、两份都留 |
| 2026-07-30 | S3 接口与前端 | 已完成 | 会话路由 37 → 7；`SessionRelay.vue` 962 → 648 行，删除 `SessionSaveDrawer.vue`；`shared/sessions.ts` 853 → 61 行，响应契约集中到 `shared/session-sync.ts` | `vue-tsc`、`tsc`、`npm run build` 通过 |
| 2026-07-30 | S4 旧机器下线 | 已完成 | 删除 epoch、lifecycle、lineage、event-store、checkpoint(-jobs)、handoff、summary、provider-summary、source-sync、cmux、probe、native-capsule、native-restore、recovery、fork、deletion-conflict、secrets、backup-all、catalog、vault 等 53 个文件 | `src` 总量 35,269 → 18,489 行；全量测试 232 项通过 |
| 2026-07-30 | S5 真机冒烟 | 已完成 | 本机 66 条真实会话一次同步 8.6 秒全部备份；二次同步 65 条跳过、1 条更新；接口冒烟测试覆盖设置 → 列表 → 预览 → 同步 → 删除 | 备份仓 363 MB / `.git` 161 MB；单会话超过 20 MB 时同步结果给出体积提示 |
| 2026-07-30 | S6 列表可读性 | 已完成 | 会话标题在 provider 未写标题或写了 `auto` 占位符时回落到第一句真实提问；预览与标题都跳过 `<task-notification>`、`<environment_context>` 等注入伪标签（按「标签名含连字符或下划线」判定，不误伤 `<div>`） | 本机 66 条真实会话：无标题项 25 → 11，Codex 的 `auto` 占位标题全部换成真实提问；预览不再被工具回执淹没 |
| 2026-07-30 | S7 扫描开销 | 已完成 | `discoverSessions` 新增 `only` 定向查找（按文件名命中，不匹配的文件不打开）与元数据缓存（>256 KB 的文件按大小+修改时间失效）；同步循环新增「备份后未修改过就跳过」快速通道，`DiscoveredSession` 补 `modifiedAt` | 本机 66 条 / 363 MB 实测：单条预览 1.05s → 0.01s，列表 1.17s → 0.02s，无变化同步 1.15s → 0.04s |
| 2026-07-30 | S8 去掉死代码 | 已完成 | 删除无人调用的 `discoverClaudeSessions` / `discoverCodexSessions`；列表接口去掉客户端从不使用的 `search` / `provider` 查询参数；`sessionSyncActionSchema` 改为普通联合类型；仅内部使用的符号取消导出 | `tsc`、`vue-tsc`、`npm run build` 通过；233 项测试全绿 |
| 2026-07-30 | S9 首次同步与结构合并 | 已完成 | 备份写入支持直接从源文件流式拷贝（不再把几十 MB 读成字符串再写出），单边存在的关系改为按存在性判定、不读内容；三处重复的「写本机内容进备份」合并为 `backupFromLocal`；删除只有测试在用的 `joinTranscript`，去掉多余 `stat`，冲突提交信息改用中文；删除随 cmux 一起下线的 `config/cmux.example.yaml` | 全新备份仓首次同步 66 条 / 392 MB：8.6s → 7.1s，其中 5.8s 是 `git add + commit` 本身（实测），应用层已压到约 1.3s，不再继续优化 |
| 2026-07-30 | S10 继续会话与权限参数 | 已完成 | 恢复 `shared/provider-command.ts` 并新增 `buildResumeCommand`：详情抽屉给出 `cd <项目> && claude --resume <id>` / `codex resume <id>`，带「跳过权限确认」勾选（Claude `--dangerously-skip-permissions`、Codex `--dangerously-bypass-approvals-and-sandbox`），选择记在 localStorage；Fleet 只生成命令不执行 | 两个参数均对照本机实装 CLI 的 `--help` 核对；7 个单测覆盖两种 provider、两种模式与 shell 注入引用 |
| 2026-07-30 | S11 Claude 项目目录解码 | 已完成 | 连字符目录名（`-Volumes-dev-wwwroot-moo-git-fleet`）改为沿真实目录逐层取「存在的最长一段」还原；还原不出来时标为未识别项目，不再给出猜测路径 | 修复前复制出来的命令会 `cd /Volumes/dev/wwwroot/moo/git/fleet`（不存在）；修复后本机 66 条会话路径全部正确，无未识别项 |
| 2026-07-30 | S12 并发保护与空状态 | 已完成 | 备份仓的三个写入口（同步、处理冲突、删除并移出备份）统一走 `withBackupLock` 串行队列；列表区分「本机一条会话都没有」与「筛选后为空」两种空状态 | 修复前并发同步会有 409（git index.lock 冲突），真机三并发实测全部成功且串行；新增 2 个并发用例 |
| 2026-07-30 | S13 换新电脑真机验证 | 已完成 | 恢复会话到本机时连同备份时间一起还原文件 mtime（`cp -p` 语义）——否则恢复出来的文件永远比备份新，「没动过就跳过」的快速通道在这台电脑上永远命中不了 | 真机全流程：A 机推 161 MB 到私有仓，B 机（空的 Claude/Codex 目录）一次同步恢复 67 条 / 371 MB，耗时 4.2s，零待决定项；抽查 3 个文件与原文 sha256 一致；B 机第三次同步 0.12s（修复前每次 2.78s） |
| 2026-07-30 | S14 离线可用与墓碑保全 | 已完成 | `fetchBackupRemote` / `pushBackup` 不再抛错，改为返回原因：连不上私有 Git 时本机备份照常完成并如实提示，落下的提交下次一起带上；对齐远端前先记录本机墓碑，对齐后把仍比远端新的补回（远端更新则听远端的）；`pushBackup` 在远端已有同一提交时跳过，离线且无改动不再白报错 | 修复前离线时 fetch 失败会中断整次同步、本机备份根本不会发生，且离线删除会被下次同步撤销；新增 3 个用例（离线备份、复网补推、离线删除不被撤销），共 246 项测试通过 |
| 2026-07-30 | S15 备份仓选择安全护栏 | 已完成 | `ensureRepository` 不再接受任何现有 Git 仓库：只允许空仓库或 Fleet 自己建过的备份仓（认 `fleet.json` 标记，工作树被清空时回退查历史），其余一律拒绝并说明原因；没有提交的空仓库统一把 HEAD 切到 `main`，避免本地 `master` 与推送目标对不上 | 修复前用户在设置里选中自己的笔记仓 / 代码仓，第一次同步的 `reset --hard` + `clean -fd` 会抹掉其中未提交改动与未跟踪文件；新增 3 个用例（拒绝有内容的仓、接受空仓并切 main、自己的备份仓可重连） |
| 2026-07-30 | S16 项目身份按强弱合并 | 已完成 | 写备份时按 `remote:` > `local:` > `unknown:` 取强者：本机认不出项目时不会用更弱的身份覆盖备份里已有的远端推导 id | 修复前在未登记该项目的电脑上续写会话，会把跨机器可用的 `remote:` id 降级成本机 `local:` id，第三台电脑就再也对不上项目 |
| 2026-07-30 | S17 同步进度可见 | 已完成 | 同步期间顶部常驻「正在同步会话 · 已用 N 秒」，超过 3 秒补一句首次同步较慢的说明；纯前端计时，服务端零改动 | 首次同步 7 秒不再是静默等待 |
| 2026-07-30 | S18 写入中的会话不产生假冲突 | 已完成 | 统一「只处理写完整的行」：没有换行结尾的尾巴既不写进备份也不参与比对（`completeContent`）；备份写入先探最后一个字节，整文件才走 `copyFile` 快路径，否则读出来截断后再写 | 修复前 Claude 正在写时同步会把半行存进备份，那行写完后被判成 diverged，弹出根本不存在的冲突；新增 5 个用例（含"整个文件只有半行"） |
| 2026-07-30 | S19 备份仓损坏的提示 | 已完成 | `requireBackupBinding` 增加「目录还在但已不是 Git 仓库」的检查，给出可操作提示而不是透传 `fatal: not a git repository` | 手动删掉备份仓 `.git` 后实测提示为「请在设置里重新连接备份位置；本机会话不受影响」 |
| 2026-07-30 | S20 比对拆成存在性与内容两步 | 已完成 | `compare.ts` 拆出 `presenceRelation`（只看有没有）与 `contentRelation`（逐行比前缀），`sessionRelation` 变成两者的组合；同步循环不再用"传空数组当存在标记"的写法 | 同步循环读文件的条件与关系判定一一对应，规则集中在一处；75 项会话测试通过 |
| 2026-07-30 | S21 按需读取与延迟解析 | 已完成 | 处理冲突时按选择读内容（跟随删除两边都不读），重复的空值检查收进 `requireLocal` / `requireBackup`；Claude 项目路径的磁盘还原改为确认目录里有目标会话后才做 | 「跟随删除」不再为一条几十 MB 的会话白读两遍；定向查找不再为无关目录做 stat 遍历 |
| 2026-07-30 | S22 同步结果色调 | 已完成 | 只有「有待决定项」或「配了远端却没推上去」才显示为警告；超大会话体积这类纯告知说明照常显示但不变警告色 | 实测一次全新备份的完成提示回到成功色，说明文字保留 |
| 2026-07-30 | S23 接口一致性与测试补齐 | 已完成 | 审计服务端 7 条会话路由与客户端调用一一对应、无孤儿；客户端 `localSessions()` 去掉服务端已不再读取的 `search` / `provider` 参数；补 `local-sessions.test.ts`（5 例：未设置备份时的列表、四种备份状态、只在备份里的计数、预览、404） | 会话子系统各模块均有单测；261 项测试通过 |
| 2026-07-30 | S24 1024px 验收 | 已完成 | 按 AGENTS.md 的视口规则在 1024×820 下验收新增 UI：详情抽屉 860px、继续命令代码块不溢出、页面无横向滚动、底部按钮完整 | README 同步补充「继续会话命令」与「备份仓只能是空目录或专用备份仓」两条 |
| 2026-07-30 | S25 发布链路核对 | 已完成 | 核对打包脚本、macOS 端到端脚本与 CI 是否引用了已删除的会话接口——只用到 `/api/health`，`mac-entry.ts` 只依赖 `buildApp`；`tsup --config tsup.mac.config.ts` 构建通过；`docs/OPERATIONS.md` 中的 "session" 均指本机鉴权会话，无需改动 | 删掉 53 个模块与 30 条路由后，发布链路无残留引用 |
| 2026-07-30 | S26 该同步了的提醒 | 已完成 | 状态条在「距上次同步超过 12 小时且确实有未备份会话」时转为提醒色，显示「有 N 条还没备份 · 上次同步 X 前」；12 小时对应上班/下班各同步一次的节奏，刚同步完不会因为活跃会话又变化而立刻唠叨 | 实机验证：造一个两天前的同步记录 → 提醒「有 67 条还没备份」；点完同步 → 立即回到正常色（此时已有 1 条活跃会话变化） |
| 2026-07-30 | S27 安装备份自动收尾 | 已完成 | 安装器成功安装后清理更旧的备份，默认保留最近 2 份（`MOO_FLEET_INSTALL_BACKUP_RETENTION` 可调），只删自己按时间戳命名的 `Moo Fleet.app.backup-*` 且绝不碰当前 App；e2e 测试成功时把预留的安装前 App 归入正常备份池（`backup-<stamp>-e2e`），不再留下永不回收的 `pre-install-e2e-*` | 修复前 `/Applications` 累积 60 份版本备份 + 7 份 e2e 现场，共 6.03 GB（本轮已手工清理）；`npm run test:mac-native` 新增保留策略用例：造 4 份假备份验证只留 2 份、四份全清、当前 App 完好且签名有效、保留份数设大时不误删 |
| 2026-07-30 | S28 统一「几分钟前」并修 NaN | 已完成 | 仓库工作台与 AI 会话页各有一份 `relativeTime`：前者遇到无法解析的时间会显示 “NaN 秒前”。抽为 `src/client/relative-time.ts` 并修掉该问题；两处原本就不同的空值占位（`—` / `时间未知`）与超过 30 天的写法（数天数 / 换日期）用参数保留，不顺手改动各自页面的现有显示 | 6 个用例覆盖逐级取整、两种 longAgo、非法时间、空值占位、未来时间；267 项测试通过 |
| 2026-07-30 | S29 升级夹具不再钉死 0.1.2 | 已完成 | e2e 第 3 轮原先要求 `/Applications` 里恰好躺着一份 0.1.2 备份作升级夹具——S27 的备份清理（以及新的保留策略）会让这个依赖失效。改为自动挑选任一与候选版本不同的旧备份，版本号与 build 从夹具自身读取，断言与日志随之参数化 | 语法检查通过；夹具解析实测选中现存的 0.1.8（build 108）。完整 5 轮真实安装需要显式 `MOO_FLEET_INSTALL_E2E_CONFIRM=1`，尚未执行 |

## 129. AI 会话页视觉与操作体验

> 当前状态：真机 1512 / 1024 双视口实测，按仓库工作台的既有尺度收敛，全部为减法

### 129.1 本轮改动

| 问题 | 处理 |
| --- | --- |
| 顶栏与页面各有一个「同步会话」按钮，点的是同一个函数 | 删掉页面里那个；顶栏是固定的，滚到长列表底部时仍然够得着，页面里那个反而会滚走 |
| 会话页用「落地页」式大标题（54px + eyebrow + 说明段落），仓库页没有 | 压成一行：17px 标题 + 同行说明，去掉 eyebrow |
| 每行末尾的箭头与右侧「查看会话」眼睛按钮功能重复 | 删掉箭头 |
| 时间/记录列固定占 402px，实际内容只需约 90px | 改为按内容宽度并右对齐，省下的宽度还给标题列（730px → 1055px，长标题少截断） |
| 概览卡片 92px 高、34px 数字，1024px 下折成 2×2 吃掉 210px | 对齐仓库页 `.summary-block` 的尺度（68px / 20px），去掉强制两列的断点 |
| 标题压成一行后，1180px 断点仍把页头竖排 | 去掉该规则，交给 `flex-wrap` 自然处理 |

### 129.2 实测

| 视口 | 首行会话位置 | 视口内可见会话 |
| --- | --- | --- |
| 1512 × 950 | 428 → 361 | 7 → 8 |
| 1024 × 820 | 545 → 384 | 3 → 6 |

两个视口均无横向滚动；267 项测试、`vue-tsc` 与构建通过。

### 129.3 操作路径实测（第二轮）

| 问题 | 处理 |
| --- | --- |
| 打开会话详情停在最早的消息，200 条要手动滚到底才看得到"我最后在干什么" | 加载完成后直接定位到最新一条（先退出加载态再滚，否则对话尚未渲染，`scrollTop` 无效） |
| 设置弹窗只问远端地址，**从不显示备份会建在哪**：留空直接确认，几百 MB 就落在用户没见过的路径上 | 弹窗内增加只读的位置说明：绝对路径 + 会话条数与体积 + "Git 会另存一份压缩副本，实际占用约为两倍"。不加输入框、不加选项，只把已有信息显示出来 |
| `FolderOpen` 图标未导入，`vue-tsc` 把未知组件当自定义元素放过 | 补上导入；此类问题只能靠真机查 DOM 发现 |

> 本轮验证过程中曾误在真实数据目录建出备份仓（584 MB）并完成一次同步，已按三重校验（目录存在 / `fleet.json` 标记 / 标记内容匹配）删除并复核 `configured: false`、67 条会话零读取错误。"设置弹窗不显示备份位置"这一条正是由此暴露。

### 129.4 待决策路径实测（第三轮）

用临时 `GIT_FLEET_HOME` 造出真实分叉（两台电脑在同一条会话第 3 行起各写各的），在界面上完整走了一遍。

| 问题 | 处理 |
| --- | --- |
| 三个按钮让人决定「用这台 / 用另一台 / 两份都留」，但**界面上看不到另一台写了什么** —— 在看不见内容的情况下做覆盖决定 | 待决策卡片内联显示分叉之后两边各自的第一句（`localFirstDiff` / `backupFirstDiff`），不新增抽屉或弹窗 |
| 选「两份都留」后列表出现两条一模一样的行，分不清哪条是副本（标题取自会话内容，两份相同） | 副本 ID 由我们生成，带 `--<设备名>` 后缀；`isKeptCopy()` 判定后在行内加「副本」标记。provider 的会话 ID 是 UUID，不含 `--`，判定不会误伤 |

实测：分叉卡片显示「这台：手机号要支持 +86 前缀 / 另一台：改用国际号码库统一校验」，点「两份都留」后列表出现带「副本」标记的第二条。

### 129.5 剩余路径实测（第四轮）

用 10 条合成会话（21 MB）在临时 `GIT_FLEET_HOME` 下把剩下的路径走完。

| 路径 | 结果 |
| --- | --- |
| 同步进行中的反馈 | 进度条正常出现（「正在同步会话 · 已用 N 秒」），同步期间顶栏按钮禁用；21 MB 不到 0.5 秒完成 |
| 删除并移出备份 | 勾选后确认按钮从「移到本机废纸篓」变为「删除并移出备份」；执行后本机文件进系统废纸篓、备份 `.jsonl` 移除、`.json` 变墓碑（保留标题）、Git 留下一条删除提交 |
| 1512 宽屏详情抽屉 | 固定 860px（占屏 57%），正文 741px，阅读宽度合适；打开即定位到最新一条 |
| 本机零会话的空状态 | 文案正确；顺手隐藏了此时无用的搜索框与 Claude/Codex 筛选器（筛选后为空时仍保留，否则没法清筛选） |

> 测试产生的合成文件曾进入系统废纸篓，已按固定名字模式清理，废纸篓无残留。

## 130. 仓库工作台的真机复核

> 用真实的 23 个仓库只读浏览（不触发任何 Fetch / Pull / Push），1512 与 1024 双视口

| 问题 | 处理 |
| --- | --- |
| `未设置 upstream` 状态徽章 168px，超出 145px 的状态列被切成「未设置 upstre…」，该行的「查看详情」箭头也被挤出单元格 | 其余状态标签都是 2–5 个汉字，这一个混入拉丁词才成为异类；改为「未关联」（95px）。完整说法保留在旁边的「分支 / Upstream」列、按钮 `title` 与 `aria-label` 中，徽章仍是可点的一键关联按钮 |
| 1024px 下状态列只分到 10%（98px），23 行全部溢出；置顶列 45px 也切掉 7px 的按钮热区 | 在既有的 1120px 断点里把状态列改成固定 150px、最近提交 22% → 19%、置顶列 56px。1512px 的列宽不受影响 |

复核结果：两个视口下 23 行零溢出、无横向滚动；仓库详情抽屉（工作区信号 / 安全操作 / 文件变化 / Stash / 最近提交 / 底部工具）信息组织清晰，未发现问题。

### 130.1 仓库页交互复核与快捷键分流

只读走了筛选排序、批量条、操作记录与快捷键面板。前三者未发现问题：三个下拉都带实时计数（分组 8 类、状态 7 档），批量按钮的 `title` 说明可执行范围，操作记录面板有批次进度、成功/跳过/失败分色、三重筛选与 SSE 实时标记，100 条操作聚合成 5 个批次数量吻合。

发现的问题：**快捷键面板列的 5 个快捷键，在 AI 会话页有 2 个是死的**。`⌘K` 去 focus 仓库搜索框（那页没有），`R` 调仓库刷新，两者都毫无反应；`H` 打开的是仓库专属的操作记录，而顶栏在会话页特意隐藏了该入口。

| 快捷键 | 处理 |
| --- | --- |
| `⌘/Ctrl + K` | 按当前工作区分流：会话页聚焦并全选会话搜索框，仓库页保持原行为 |
| `R` | 会话页重新扫描本机会话，仓库页保持原行为 |
| `H` | 会话页直接忽略，与顶栏隐藏该入口一致 |
| 帮助面板文案 | 「搜索仓库 / 刷新本地状态」改为「搜索当前页面 / 刷新当前页面」，`H` 标注「（仓库舰队）」 |

四种组合均实测通过（`R` 一度看似无效，实为探针假阴性：扫描只需约 20 ms，转圈早已结束，改用请求计数确认确有重新扫描）。

### 130.2 文案与排版全面审核（第 3 轮）

系统抽取 738 条用户可见中文文案，按术语一致性、标点、英文残留三个维度过了一遍。

| 维度 | 结论 |
| --- | --- |
| 术语 | 「私人仓库」×3 统一为「私有仓库」（与服务端提示「私有 Git 仓库」及 GitHub/Gitee 行业术语一致）；「本机会话」26 处已统一，无「本地会话」混用 |
| 标点 | 中文文案零半角标点混用；句号规律一致（完整句带句号，短语式标签不带） |
...
### 130.4 弹窗审核与「移出」用词统一（新循环第 2 轮）

只读审了三个此前未检查的弹窗：**移出确认**（说清移除什么 / 保留什么 / 如何恢复，结构好）、**编辑仓库配置**（隐私策略说明条 + 能力开关六宫格，无问题）、**分支切换面板**（「只允许干净工作区；不会自动 Stash 或强制覆盖」+ CURRENT 徽章，无问题）。

发现并修复：入口按钮叫「移出列表」，确认弹窗标题与确认按钮却叫「移出工作台」——同一动作两个名字。统一为「移出工作台」（工作台是产品概念，列表只是呈现形式）；管理弹窗底部说明同步改为「把仓库移出工作台不会删除代码」。真机确认按钮已更新，268 项测试全绿。

### 130.5 API 断连错误态实测（新循环第 3 轮）

单独停掉本地服务进程（保留前端），完整走了断连 → 呈现 → 恢复三段：

| 观察点 | 结果 |
| --- | --- |
| 仓库页断连 | 顶栏灯转红（LOCAL API / ERROR），列表区换成「本地状态扫描失败 + 重新扫描」错误面板 |
| 会话页断连 | 「会话读取失败 + 重试」错误面板 |
| 恢复 | 会话页点重试即回 67 条；仓库页下一轮自动扫描自愈（READY + 23 行），无需手动干预 |
...
`npm run build` + `npm start` 起自托管生产版走查：本轮循环全部改动均已进打包产物——9 项新文案抽查命中、零旧文案残留（私人仓库 / 移出列表 / 未设置 upstream 徽章均已不在 JS 产物中）；23 仓库 / 67 会话渲染正常，会话抽屉打开即最新、继续命令区在位、无横向滚动。

本循环（界面排版 + 文案优化）5 轮小结：1920 验收补齐、扫描候选标注会话备份仓、三弹窗审核与「移出工作台」用词统一、API 断连三段实测与后端失联文案修复、Stash 面板守门与排序行为确认、生产走查。共 6 个提交，均已推送双平台。
...
- **更换备份位置入口**：已配置后状态区新增「更换备份位置」小按钮（此前设置完就再也打不开设置弹窗，无法从"只备份在本机"升级为跨电脑同步）；弹窗在已配置时显示当前位置、预选当前项，标题/提交按钮切换为「更换备份位置 / 保存并备份」，并注明原位置备份不会被删除。
- **备份分支跟随仓库自身分支**：不再强制 main。clone 自「默认分支 master 的空私仓」（Gitee 默认）时保持 master，fetch/push/remote-tip 全部按当前分支走；仅 Fleet 自建仓库或异常态用 main。解决了远端多出一个没人要的 main 分支的问题（131 节遗留边界闭环）。
- **README 陷阱提醒**：引导文案明确「建仓时不要勾选初始化 README」，避免小白 clone 下来不是空仓库被守卫拒绝。
- 验收：276 测试全绿（新增 master 分支全链路往返等 3 条），typecheck / build 通过；真机核对未配置态渲染与文案。分工照旧：Opus 实现，Fable 验收 + 标题/按钮文案的机械性补充。

### 131.2 双机全流程真机走查（隔离环境）

用 GIT_FLEET_HOME / GIT_FLEET_CLAUDE_HOME / GIT_FLEET_PORT 起了两套完全隔离的服务（8801/8802），远端用本地裸仓库模拟「默认分支 master 的 Gitee 空私仓」，把小白的完整故事走了一遍：

1. **A 机首选「只备份在本机」**：初始化 + 同步，2 条会话备份进数据目录，无远程不推送 ✓
2. **A 机「更换备份位置」到 clone 下来的空私仓**：origin 自动读出、同步推送成功；远端只有 `refs/heads/master`，没有多余的 main（131.1 分支跟随的端到端实证）；原位置备份原样保留 ✓
3. **B 机 clone 同一个仓库并选中**：一次同步「备份 1 条 · 恢复 2 条」，恢复文件与 A 机原件逐字节一致 ✓
...
### 136. AI 会话列表与备份弹窗排版优化（用户反馈）

> 当前状态：备份位置弹窗、会话页顶部工具栏与会话列表项目排序均已完成

- 备份位置弹窗改为单一内容滚动区，底部操作区固定；宽度提升到 720px，选项卡片和目标位置预览统一为紧凑、可省略但可悬停查看完整路径的布局。
- 会话页标题与辅助描述改为两行，和右侧状态 / 更换位置 / 刷新控件垂直对齐；控件统一 44px 高，工具栏收紧到 60px，并压缩顶部空白与概览间距。
- 会话列表新增「最近活动 / 按项目」排序；按项目名称分组，同项目内按最近活动倒序，同名项目按路径稳定排序，未识别项目置后；搜索与 Provider 筛选保持组合生效。
- 验收：1024×768、1024×512 与 1440×900 桌面视口无裁切、换行或横向溢出；`npm run typecheck`、`npm run build`、`npm test`（51 个测试文件 / 310 项）及 `git diff --check` 通过。开发环境偶发的 `/api/batches` 403 属本地 origin 白名单配置，不影响本轮 UI 行为。

### 136.1 仓库页顶部栏窄桌面排版（用户反馈）
