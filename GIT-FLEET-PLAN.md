...
- 托盘、开机启动、系统通知。
- GitHub / Gitee commit 或 PR 链接。
- 长期未 Fetch、长期未 Push、默认分支不一致、remote 不可达等健康指标。
...
- [x] 百仓库级扫描压测和大 diff 限流：临时仓库压力脚本验证 100 仓库扫描、顺序稳定和状态正确；Patch 流式限为 120KB，staged 指纹改用完整 index tree hash。
