...
> 当前状态：本机会话管理 + 私有 Git 备份恢复；权威使用与技术说明见 `docs/AI-SESSION-SYNC.md`

### 127.1 当前业务边界

- 自动发现、搜索、排序和预览本机 Claude / Codex 会话；详情提供可复制的继续命令，Fleet 不直接启动 provider。
- 本机 JSONL 是真相，备份仓是派生副本；同步只处理会话原文与恢复所需元数据，不处理项目源码、WIP、交接摘要、登录态、API Key 或机器配置。
- 一次“同步会话”完成远端接收、本机与备份前缀比对、自动追加、一次提交和一次推送；只有真分叉或另一台已删除而本机仍存在时要求用户选择。
- 删除默认只把本机会话移进系统废纸篓；显式选择后才在备份中写墓碑，另一台同步时仍需确认，不会静默删除现有本机文件。
- 恢复时优先用 Git 远端推导的 `projectId` 定位本机项目；找不到时按备份相对路径落地，不维护跨电脑路径映射表。
- 备份位置可以是 Fleet 数据目录、扫描根中的新版备份仓 / 旧版 Vault / 空 Git 仓库，或通过原生目录选择器指定的其他文件夹；远端只读取所选仓库自身的 `origin`。

### 127.2 当前代码与接口

| 职责 | 当前入口 |
| --- | --- |
| 会话发现与预览 | `src/server/sessions/discovery.ts`、`local-sessions.ts`、`content-preview.ts` |
| 内容关系 | `src/server/sessions/compare.ts`：`same / local-only / backup-only / local-ahead / backup-ahead / diverged / backup-deleted` |
| 备份文件 | `src/server/sessions/backup-store.ts`：`sessions/<provider>/<id>.jsonl + .json` |
| 备份仓与候选 | `src/server/sessions/backup-repo.ts` |
| 同步、选择与删除 | `src/server/sessions/sync-run.ts` |
...
- 备份仓只能是空目录、无提交的空 Git 仓、新版 Fleet 备份仓，或经用户明确确认升级的可识别旧版 Vault；普通有内容仓库始终拒绝。
- 写操作统一串行；会话 ID 必须通过路径安全校验；正在写入但尚无换行的尾行不进入备份也不参与比对。
- 同步对齐远端前保留更新的本机墓碑；远端不可用时仍完成本机备份并返回原因，下次联网继续推送。
- 新版备份仓使用当前分支 fetch / push；旧版 Vault 的覆盖升级只清理工作树，旧内容仍留在 Git 历史。
...
### 138. 仓库工作台标题与筛选区布局优化（用户反馈）

> 当前状态：已完成

- 标题、结果摘要、扫描状态与重置动作收拢为第一层信息；搜索、排序、分组与状态筛选独占第二层工具栏，避免标题信息和四个操作控件挤在同一行。
- 搜索框前置并弹性占据剩余宽度，排序、分组、状态筛选保持原选项、计数、持久化与组合筛选语义；批量操作范围与 Git 操作不变。
- 文件范围限定为 `src/client/App.vue`、`src/client/styles.css` 与本节计划记录；验收覆盖 1024px 与 1440px 桌面视口、搜索/下拉交互、横向溢出、类型检查和前端构建。
- 验收：1024×768 与 1440×900 真浏览器下页面及标题区无横向溢出；搜索、清空、重置与排序下拉选择正常，控制台 0 error / 0 warning；`npm run typecheck`、`npm run build:client` 与 `git diff --check` 通过。
- 用户复核修正：重置按钮原固定 78px，在实际中文字体下不足以容纳图标、间距与「重置条件」，且放在结果摘要 / 扫描时间层既不显眼也不符合操作语义；最终移到筛选工具栏最右侧，改为与筛选控件同高的绿色次级动作「重置筛选」，仅在有条件时出现。1024×768、23 仓库且「待拉取」筛选激活时实测为 96×38px，工具栏和页面均无横向溢出。

### 139. 全局提示条图标去重（用户反馈）

> 当前状态：已完成

- 根因：警告消息沿用 `⚠` 文案前缀表达语义，全局提示条又按该前缀渲染 `AlertTriangle`，因此同一条提示出现两个警告图标。
- 展示层统一把成功、警告、错误归一为 tone、纯文案与停留时间；警告前缀仅作为兼容输入，不再进入可见文案。服务端消息与操作记录语义不变。
- 检查范围覆盖前端本地产生的警告、Commit / Push 服务端警告，以及成功、错误两种全局提示；验收包含单元测试、类型检查、前端构建和 1024px / 宽屏真实页面。
- 验收：新增 5 条展示测试，成功 / 警告 / 错误均为 1 个状态图标 + 纯文案 + 1 个关闭按钮；1024×768 与 1440×900 真浏览器下提示完整位于视口内、页面无横向溢出，错误提示保持 `alert` / `assertive`，控制台 0 error / 0 warning；`npm run typecheck`、`npm run build:client`、`npm test`（52 个文件 / 315 项）及 `git diff --check` 通过。

### 140. 操作记录入口去重（用户反馈）

> 当前状态：已完成

- 顶部「操作记录」保留为仓库工作区唯一入口，移除批量工具栏右侧重复的完成态 `FETCH/PULL/PUSH N/N` 按钮。
...
### 142. AI 会话管理操作体验收口（用户反馈）

> 当前状态：已完成

- **备份位置安全**：更换位置时始终补回并预选当前 `backupPath`；未变更时禁用保存，从带远程的备份仓切到明确无远程目录时要求确认“跨电脑同步将停止”。不改变后端写入守卫和同步语义。
- **状态直接可操作**：只要存在新增或有新内容的会话，状态立即显示为待备份，顶栏同步按钮携带数量；概览的“这台电脑 / 等待备份”成为组合筛选入口，“需要你决定”在同步前显示为尚未检查。
- **列表稳定与状态保留**：30 秒静默刷新只更新数据，新会话置顶但既有行不因活动时间变化而跳位；手动刷新重新采用最新顺序。AI 会话组件通过 KeepAlive 保留搜索、Provider、排序、备份筛选和同步待处理项。
...
> 当前状态：实现、本机双架构验证与 GitHub Intel 真机验收均已完成；0.1.16 版本升级和发布不在本节自动执行
...
- **R0 本机源码、双架构与 arm64 安装验收**：发布提交 `ad70d00` 通过 `npm run typecheck`、单 worker 全量测试（56 个文件 / 333 项）、`npm run build`、arm64/x64 原生专项、`npm audit --omit=dev`（0 vulnerabilities）、双架构 DMG 构建、签名与 `hdiutil verify`。arm64 五回真实 `/Applications` 安装 5/5 通过，0.1.16 → 0.1.17 升级保持配置不变，最终安装态健康运行。arm64 DMG 为 44,840,266 bytes，SHA-256 `3895c1356b47cc3a6b75122ddf02f2c4a885347e64fc067f5ba0be91d5bc7250`。
- **R1 Intel runner 真机验收**：GitHub `macos-15-intel` 在提交 `ad70d00` 的 [run 31668444466](https://github.com/charsen/moo-git-fleet/actions/runs/31668444466) 上通过 Intel runner 识别、typecheck、单 worker 全量测试、x64 原生专项、生产依赖审计、x64 DMG 构建、五回真实 `/Applications` 安装及 artifact 上传。artifact `9168931668` 回下载后为最终 x64 DMG（47,127,507 bytes），SHA-256 `a69475cfa0c49b68c2086ce78ab3ec6796aff03e19779816f52c45fe0f903b18`；App `0.1.17` / build `117`，Swift/Node 均为 x86_64，Node `v24.18.0`，签名与镜像 checksum 有效。
- **P0 双仓源码、tag 与 Release 发布**：发布提交 `ad70d00` 已作为发布时的 `dev`、`master` 和 annotated tag `v0.1.17` 同步到 Gitee 与 GitHub，临时 `intel-validation/v0.1.17` 分支已从双仓删除。Gitee Release `795080` 与 GitHub Release `369677077` 均上传 arm64/x64 两份 DMG；四个公开附件回下载后的字节数、SHA-256 均与冻结候选一致，`hdiutil verify` 全部通过。Release：`https://gitee.com/charsen/moo-git-fleet/releases/tag/v0.1.17`、`https://github.com/charsen/moo-git-fleet/releases/tag/v0.1.17`。

### 149. macOS 15 Dock 图标圆角修复与发版 0.1.18

> 当前状态：源码、annotated tag、GitHub Release 与双架构 DMG 已发布；Gitee Release 待补，发布后由同事在 macOS 15.3.1 目标机复验
...
> 当前状态：本地发版准备与双架构制品已完成，等待远端推送与 Release
...
- 发版动作：版本 `0.1.21` → `0.1.22`（`package.json`、`package-lock.json` 两处、`scripts/macos-internal-install-readme.txt`）；发布提交 `1731d5e`；annotated tag `v0.1.22`（消息 `Release v0.1.22`）；`dev` 快进合并到 `master`（`master` 是 `dev` 的祖先，无分叉，未产生 merge 提交）。现在 `dev` 与 `master` 同指 `1731d5e`，tag 对两个分支均可达。
- 双架构制品（ad-hoc 内测签名，未公证）：
  ...
  - 两个 App 均为 `0.1.22` / build `122`、bundle ID `com.mooeen.moofleet`；App 可执行与内置 Node 架构匹配（arm64 / x86_64）；内置 Node `v24.18.0`；内测安装说明首行已同步为 `Moo Fleet 0.1.22 内测安装说明`。
- 待办（尚未执行）：推送 `dev`、`master`、tag `v0.1.22` 到远端；创建 Release 并上传两份 DMG；回下载校验。真实安装 E2E 未运行。
