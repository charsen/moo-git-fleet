...
- 起因：第 161 节的实现被用户判定为「把交互变复杂了」——要多点一次「批量选择」才能进入多选，工具栏还多出三个按钮。
- 改动：
  - **勾选框默认可见**，删掉「批量选择 / 退出选择」开关与 `batchSelectionMode` 状态。
  - **全选移到表头那一格**：`#` 列的表头变成勾选列，支持三态（全选 / 半选 / 未选）。
  - 删掉「全选当前结果」与「清空」按钮；原「清空」的位置改为显示「已勾选 N 个仓库」。
  - 行号随 `#` 列被勾选框取代而移除；该列宽度（42px）与表格 `table-layout: fixed` 的 `nth-child` 列宽都不受影响。
