...
- 仓库详情把当前状态和收藏按钮紧邻项目名称展示，标题信号栏集中显示分支、改动、Ahead / Behind、Fetch、Stash 和扫描时间；工作区信号后紧跟安全操作，文件变化置于其后。仓库资料、Git 身份和本地打开方式收进底部紧凑 Dock，仓库主页与查看提交入口分别贴近 Remote URL 和最近提交，低频 Stash 默认折叠。
