...
最终制品门禁（2026-07-29）：`npm run typecheck`、`npm test`（64 个文件 / 265 项）、`npm run build`、`npm audit --omit=dev`（0 vulnerabilities）、`npm run test:mac-native` 和 `npm run build:mac` 均通过；最终 App `0.1.8 / build 108`、Node `v24.18.0`、App/Node strict codesign 与 `hdiutil verify` 通过。主路径降噪的源码变化后候选再次清零，五回真实 `/Applications` 安装从第 1 回重新计数并全部通过，最终安装态健康端口为 `19792`；DMG `release/Moo-Fleet-0.1.8-macos-arm64.dmg` 为 41,120,899 bytes，SHA-256 为 `56b3b6d15d6fda718c6db38e0d41270ac6b9b22033bd2a83a447a2e664101e24`。从最终 `/Applications` 安装包启动仓库外隔离实例，在 1024×900 未配置状态打开单页快速开始，推荐目录自动落在 `GIT_FLEET_HOME/session-vault`；完成“明文边界确认 → 立即开始”两次点击后进入“仅本机保存”，Console 0 error / 0 warning，未读取或写入真实 Vault/provider 目录。该包为内部 ad-hoc 签名、未公证；真实 provider 登录、同版本 CLI 原生 resume 和 ≥3 条真实历史会话人工评审仍需发布者在目标环境完成。
