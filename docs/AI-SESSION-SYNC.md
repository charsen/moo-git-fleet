...
- Vault 远端必须由用户主动选择跨电脑模式并填写自己控制的私有 Git URL；客户端按固定合同提交归属确认，服务端对缺失或错误确认值继续硬拒绝。UI 常驻 `私有（用户确认，未经 Fleet 验证）` 标签；未确认时只能本机保存或轻量 handoff。平台 API 自动验证可见性是后续增强，不是 MVP 门槛。
...
顶层导航新增一个轻量的「会话接力」工作区，与现有仓库工作台并列。首次进入时显示单页快速开始：

1. Fleet 自动给出应用数据目录内的专用推荐位置；普通用户只看位置摘要，需要更换时再展开绝对路径。
2. 默认选择“仅本机”；需要跨电脑时改选“跨电脑同步”并填写自己控制的私有 Git URL。remote 名称默认 `origin`，与绝对路径一起放进高级设置。
3. 用户只需一次确认“内容以明文保存在 Git，历史与备份可能继续保留”。主动选择跨电脑模式并填写 URL 即构成远端归属确认，不再要求重复勾选或逐字输入短语；客户端仍按固定合同提交确认值，服务端继续执行完整硬校验。
4. Claude / Codex 目录由 Fleet 只读自动发现；设备名沿用个人配置，项目路径映射沿用仓库注册表，均不阻塞 Vault 初始化（§4.2）。

之后的日常流程应控制在两个主要动作内：

- **准备离开**：打开「保存并同步」→复核 Fleet 自动选中的最近可用会话与推荐源码保存结果→执行保存；只有选错时才展开“更换会话”。
- **开始工作**：点击「接着工作」→选择会话 →确认恢复预检；cmux 可用时直接打开，不可用时复制指令。

完整交接字段、源码策略细节、原生会话胶囊以及 Commit、Push、Pull、秘密扫描和 checkpoint 进度仍应按需展开，方便诊断，但不占据高频路径的按钮层级。默认界面必须继续明确显示“选中了哪条会话”和“源码将如何保存”；代码不可达风险不能因降噪而隐藏。
...
- Vault 可以放在应用数据目录中的独立 Git worktree，或用户明确选择的独立仓库；初始化时必须检查路径与远端 URL。跨电脑模式由用户主动选择并填写自己控制的私有远端，客户端按固定协议提交归属确认，服务端继续校验确认值。
...
1. **远端归属确认（MVP 为用户确认，不是自动验证）**：用户主动选择“跨电脑同步”并填写自己控制的私有 Git URL，即完成产品层的归属确认；界面不再要求逐字输入确认短语。客户端仍向服务端提交固定确认值，服务端缺少或收到错误确认值时继续拒绝初始化。成功后 UI **常驻**显示 `私有（用户确认，未经 Fleet 验证）`。通过平台 API 自动探测可见性是**后续增强**，不是 M2 的门槛——Fleet 不假装自己验证过一件它没验证过的事。
...
| 场景 | 用户操作 | Fleet 反馈 |
| --- | --- | --- |
| 下班前 | 打开 `保存并同步` →复核自动推荐→保存 | 默认展示最近可用会话与源码保存结果；摘要生成、源码同步门、扫描、脱敏、提交、Push 的逐项结果按需展开 |
...
首次设置采用单页快速开始，只保留普通用户必须理解的选择：

1. Fleet 自动填入 `GIT_FLEET_HOME/session-vault` 作为独立推荐目录，并显示易读的位置摘要；“更换”和高级设置才暴露绝对路径。
2. 默认“仅本机”可直接开始；改选“跨电脑同步”时只额外填写不含凭据的私有 Git URL。remote 名称默认 `origin`，按需在高级设置中修改。
3. 勾选一次“明文保存在 Git，历史与备份可能继续保留”后即可初始化。远端模式的选择和 URL 输入本身构成用户归属确认；客户端代填服务端要求的固定确认值，服务端仍最终复核路径重叠、嵌套仓库、同远端、凭据 URL 和确认值。
...
| M5 步骤 3：最终交付审计 | **已完成（隔离门禁）** | 两台隔离 HOME / GIT_FLEET_HOME 已完成保存 → Push → Pull → 预检 → 通用恢复五轮闭环；64 个测试文件 / 265 项、类型检查、生产构建、依赖审计、原生安装器和 0.1.8 DMG 五回真实安装均通过。真实 Claude/Codex 登录与人工会话质量仍单列，不以合成测试冒充 |
...
- API 缺少或携带错误的固定归属确认值时，远端初始化必须被服务端拒绝；产品界面不要求用户逐字输入该协议值。
...
最终制品门禁（2026-07-30）：开发分支先汇合最新 `master` 的 0.1.7 稳定性、App 图标和最近提交 tag 展示，再执行 `npm run typecheck`、`npm test`（64 个文件 / 265 项）、`npm run build`、`npm audit --omit=dev`（0 vulnerabilities）、`npm run test:mac-native` 和 `npm run build:mac`，全部通过；最终 App `0.1.8 / build 108`、Node `v24.18.0`、App/Node strict codesign 与 `hdiutil verify` 通过。新候选从第 1 回清零完成五回真实 `/Applications` 安装，最终安装态 Swift / Node / 健康端口为 `83931 / 83944 / 24659`；DMG `release/Moo-Fleet-0.1.8-macos-arm64.dmg` 为 41,120,906 bytes，SHA-256 为 `e78eea5ade2a353e9ae6dea0efa983899d6c24ebd099aff8fa7495ef849266d4`。从最终 `/Applications` 安装包的内置 Node 启动仓库外隔离实例，在 1440×1000 打开单页快速开始并切到“跨电脑同步”：界面只增加私有 Git 地址，不再出现“确认是私有仓库”复选框；页面无横向溢出，弹层打开时背景锁滚，Console 0 error / 0 warning，未读取或写入真实 Vault/provider 目录。该包为内部 ad-hoc 签名、未公证；真实 provider 登录、同版本 CLI 原生 resume 和 ≥3 条真实历史会话人工评审仍需发布者在目标环境完成。
...
- 会话仓库默认必须是用户确认的私有远端；客户端缺少固定归属确认值时阻止完整 Vault 初始化，且 UI 常驻“用户确认、未经 Fleet 验证”标签，不冒充已验证。
