...
describe('remote browser links', () => {
  it.each([
    ['git@gitee.com:example-org/moo-git-fleet.git', 'Gitee', 'https://gitee.com/example-org/moo-git-fleet'],
    ['ssh://git@gitee.com/example-org/moo-git-fleet.git', 'Gitee', 'https://gitee.com/example-org/moo-git-fleet'],
    ...
    ['ssh://git@gitee.com:22/example-org/project.git', 'Gitee', 'https://gitee.com/example-org/project'],
  ...
  it('builds provider-specific commit links and rejects unsafe hashes', () => {
    const gitee = remoteLinks('git@gitee.com:example-org/project.git');
    ...
    expect(gitee?.commitUrl('a1b2c3d')).toBe('https://gitee.com/example-org/project/commit/a1b2c3d');
    expect(gitee?.commitUrl(fullSha)).toBe(`https://gitee.com/example-org/project/commit/${fullSha}`);
  ...
  it('allows a generic repository page but does not guess its commit route', () => {
    ...
    expect(remoteLinks('/srv/projects/project')).toBeNull();
