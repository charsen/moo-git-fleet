...
describe('server shutdown', () => {
  it('closes active operation event streams before waiting for HTTP connections', async () => {
    ...
    const streamClosed = new Promise<void>((resolve) => response.once('close', resolve));
    ...
    await Promise.race([
      streamClosed,
      new Promise<never>((_, reject) => setTimeout(() => reject(new Error('SSE 连接未关闭')), 1_000)),
    ]);
